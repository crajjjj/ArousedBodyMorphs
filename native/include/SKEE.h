...
// RaceMenu's SKEE inter-plugin interface, in header form so sources aren't
// included. Acquired at runtime via SKSE messaging (InterfaceExchangeMessage
// dispatched to "skee"), so there is no link-time dependency on RaceMenu.
// The subset below matches skee64's public ABI (same header ships in several
// consumers, e.g. SexLab Inflation Framework NG).
